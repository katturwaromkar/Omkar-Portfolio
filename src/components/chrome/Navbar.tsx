      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 top-0 z-40 flex flex-col bg-[var(--bg)]/95 backdrop-blur-2xl lg:hidden"
          >
            <ul className="mt-28 flex flex-col gap-1 px-6" onClick={(e) => e.stopPropagation()}>
              {navLinks.map((l, i) => {
                const active = isActive(l.href);
                return (
                  <motion.li
                  ...
                  >
                    <Link
                      ...
                      onClick={() => setOpen(false)}
                      className={cn(
                        "flex items-center justify-between border-b border-[var(--border)] py-4 text-xl font-bold transition-colors",
                        active ? "text-[var(--brand-3)]" : "text-[var(--text)] hover:text-[var(--brand)]",
                      )}
                    >
                      <span>{l.label}</span>
                      {active && <span className="h-2 w-2 rounded-full bg-[var(--brand-3)]" />}
                    </Link>
                  </motion.li>
                );
              })}
            ...
            <a
              ...
              download
              onClick={() => setOpen(false)}
              className="mx-6 mt-8 inline-flex items-center justify-center gap-2 rounded-full bg-[linear-gradient(135deg,var(--brand),var(--brand-2))] px-6 py-3.5 text-base font-semibold text-white shadow-lg shadow-[var(--brand)]/20"
            >
