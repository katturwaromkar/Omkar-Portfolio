        ...
        <AnimatePresence mode="popLayout">
          {visible.map((p) => (
            <motion.article
              ...
              onClick={() => setActive(p)}
              data-cursor="hover"
              className="card-spotlight group glass relative cursor-pointer overflow-hidden rounded-2xl transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-[var(--brand)]/15"
            >
              {/* Visual Banner Header */}
              <div className="relative h-28 w-full overflow-hidden bg-gradient-to-r from-[var(--brand)]/20 via-[var(--brand-2)]/20 to-[var(--brand-3)]/20 p-5">
                <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,var(--brand-2)_0%,transparent_70%)] opacity-40" />
                <div className="relative flex justify-between items-start">
                  <div className="flex flex-wrap gap-1.5">
                    {p.categories.map((c) => (
                      <span
                        key={c}
                        className="rounded-md bg-[var(--bg)]/80 backdrop-blur-md px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[var(--brand-3)] border border-[var(--border)]"
                      >
                        {c}
                      </span>
                    ))}
                  ...
                  {p.featured && (
                    <span className="rounded-full border border-[var(--brand-3)]/40 bg-[var(--brand-3)]/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[var(--brand-3)] backdrop-blur-md">
                      Featured
                    </span>
                  )}
                </div>
              ...
              <div className="p-6 sm:p-7">
                <h3 className="text-xl font-bold tracking-tight text-[var(--text)] transition-colors group-hover:text-[var(--brand-3)]">
                  {p.title}
                </h3>
                <p className="mt-1 text-sm font-semibold text-[var(--brand-2)]">
                  {p.subtitle}
                </p>
                ...
                <div className="mt-5 flex flex-wrap gap-1.5">
                  {p.tech.slice(0, 5).map((t) => (
                    <span
                      key={t}
                      className="rounded-full border border-[var(--border)] bg-[var(--surface-1)] px-2.5 py-1 text-xs text-[var(--text-muted)]"
                    >
                      {t}
                    </span>
                  ))}
                </div>
                <span className="mt-6 inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--brand-3)]">
                  View case study
                  <ArrowUpRight
                    size={16}
    ...
    <AnimatePresence>
      {project && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          onClick={onClose}
          className="fixed inset-0 z-[70] flex items-center justify-center overflow-y-auto bg-black/75 p-3 sm:p-6 backdrop-blur-md"
          role="dialog"
          aria-modal="true"
        ...
        >
          <motion.div
            ...
            className="glass-strong relative my-4 max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-3xl p-6 sm:p-9 shadow-2xl"
          >
            <button
              onClick={onClose}
              aria-label="Close"
